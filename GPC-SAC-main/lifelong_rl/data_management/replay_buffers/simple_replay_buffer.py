...
def load_replay_buffer_from_snapshot(new_replay,
                                     snapshot,
                                     force_terminal_false=False):
    for t in range(len(snapshot['replay_buffer/actions'])):
        sample = dict(env_info=dict())
        for k in [
            'observation', 'action', 'reward', 'terminal',
            'next_observation', 'env_info'
        ]:
...
class SimpleReplayBuffer(ReplayBuffer):
    ...
    def random_batch(self, batch_size, min_pct=0, max_pct=1, include_logprobs=False, return_indices=True):
        indices = np.random.randint(int(min_pct * self._size), int(max_pct * self._size), batch_size, )
        batch = dict(
    ...
    def getdim(self):
        return self._observation_dim, self._action_dim, self._size
    ...
    def reset(self):
        ...
        self._logprobs = np.zeros_like(self._logprobs)

        for key, val in self._env_infos.items():
            self._env_infos[key] = np.zeros_like(val)
